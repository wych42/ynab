...
export function loadDemoData() {
  ...
  const demoCurrency = resolveInternalAccountCurrency(db) || "CNY";
  const accounts = {};
  accounts.bank = createAccount({ name: "招商银行储蓄卡", type: "checking", currencyCode: demoCurrency, startingBalance: yuan(9600), startingDate: "2026-02-25" });
  accounts.alipay = createAccount({ name: "支付宝", type: "cash", currencyCode: demoCurrency, startingBalance: yuan(680), startingDate: "2026-02-25" });
  accounts.wechat = createAccount({ name: "微信钱包", type: "cash", currencyCode: demoCurrency, startingBalance: yuan(420), startingDate: "2026-02-25" });
  accounts.save = createAccount({ name: "定期储蓄账户", type: "savings", currencyCode: demoCurrency, startingBalance: yuan(18000), startingDate: "2026-02-25" });
  accounts.credit = createAccount({ name: "平安银行信用卡", type: "creditCard", currencyCode: demoCurrency, startingBalance: yuan(-2360), startingDate: "2026-02-25" });
  accounts.invest = createAccount({ name: "指数基金组合", type: "investment", currencyCode: demoCurrency, startingBalance: yuan(52000), startingDate: "2026-02-25" });
  ...
  const assign = db.prepare("INSERT INTO assignments(currency_code,month,category_id,assigned) VALUES(?,?,?,?) ON CONFLICT(currency_code,month,category_id) DO UPDATE SET assigned=excluded.assigned");
  const setGoal = db.prepare("INSERT INTO goals(currency_code,category_id,type,target,target_month) VALUES(?,?,?,?,?) ON CONFLICT(currency_code,category_id) DO UPDATE SET type=excluded.type,target=excluded.target,target_month=excluded.target_month");

  setGoal.run(demoCurrency, cats["应急基金"], "targetBalance", yuan(30000), null);
  setGoal.run(demoCurrency, cats["旅行基金"], "targetByDate", yuan(12000), "2027-02-01");
  for (const b of ["房租房贷", "水电燃气", "订阅服务", "医疗保险"]) setGoal.run(demoCurrency, cats[b], "monthly", 0, null);
  ...
  const tx = db.transaction(() => {
    for (const m of months) {
      ...
      assign.run(demoCurrency, m, cats["食品杂货"], yuan(1400));
      assign.run(demoCurrency, m, cats["餐饮外出"], yuan(900));
      assign.run(demoCurrency, m, cats["交通出行"], yuan(300));
      assign.run(demoCurrency, m, cats["日用百货"], yuan(250));
      assign.run(demoCurrency, m, cats["话费网费"], yuan(150));
      assign.run(demoCurrency, m, cats["房租房贷"], yuan(4200));
      assign.run(demoCurrency, m, cats["水电燃气"], yuan(300));
      assign.run(demoCurrency, m, cats["订阅服务"], yuan(60));
      assign.run(demoCurrency, m, cats["医疗保险"], yuan(220));
      assign.run(demoCurrency, m, cats["应急基金"], yuan(1600));
      assign.run(demoCurrency, m, cats["旅行基金"], yuan(700));
      assign.run(demoCurrency, m, cats["医疗健康"], yuan(100));
      ...
      assign.run(demoCurrency, m, `cc:${accounts.credit}`, yuan(500));
