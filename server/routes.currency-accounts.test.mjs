...
describe("currency settings enable built-in ledgers and set reporting currency", () => {
  ...
  it("sets reportingCurrency to an enabled currency and does not rewrite amounts", async () => {
    ...
    expect(posted.status).toBe(200);
    const assigned = await call("PUT", `/api/budget/${boot.json.currentMonth}/category/${categoryId}/assign`, {
      assigned: 1500,
    });
    expect(assigned.status).toBe(200);
    const goal = await call("PUT", `/api/goals/${categoryId}`, { type: "monthly", target: 2600 });
